...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/kedro_notebook_project")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=kedro_notebook_project"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "Kedro Notebook Project"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "kedro_notebook_project"
